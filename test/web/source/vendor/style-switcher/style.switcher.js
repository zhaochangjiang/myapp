var styleSwitcher = {
    initialized: !1, options: {color: "#CCC", gradient: "false"}, initialize: function () {
        ...
        if (!this.initialized) {
            ...
            $.styleSwitcherCachedScript = function (a, b) {
                ...
                less = {env: "development"}, $.styleSwitcherCachedScript("assets/vendor/less/less.js").done(function () {
                    a.build(), a.events(), null != $.cookie("colorGradient") && (a.options.gradient = $.cookie("colorGradient")), null != $.cookie("skin") ? a.setColor($.cookie("skin")) : a.container.find("ul[data-type=colors] li:first a").click(), null == $.cookie("initialized") ? (a.container.find("h4 a").click(), $.cookie("initialized", !0)) : a.container.css({bottom: "-" + a.container.height() + "px"}), a.container.addClass("ready"), a.initialized = !0, $(window).load(function () {
        ...
        var a = this, b = $("<div />").attr("id", "styleSwitcher").addClass("style-switcher visible-lg").append($("<h4 />").html("Style Switcher").append($("<a />").attr("href", "#").append($("<i />").addClass("fa fa-cogs"))), $("<div />").addClass("style-switcher-mode").append($("<div />").addClass("options-links mode").append($("<a />").attr("href", "#").attr("data-mode", "basic").addClass("active").html("Basic"), $("<a />").attr("href", "#").attr("data-mode", "advanced").html("Advanced"))), $("<div />").addClass("style-switcher-wrap").append($("<h5 />").html("Colors"), $("<ul />").addClass("options colors").attr("data-type", "colors"), $("<hr />"), $("<div />").addClass("options-links").append($("<a />").addClass("reset").attr("href", "#").html("Reset"), $("<a />").addClass("get-css").attr("href", "#getCSSModal").html("Get Skin CSS"))));
        $("body").append(b);
        var c = '<div class="modal fade" id="getCSSModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"> <div class="modal-dialog"> <div class="modal-content"> <div class="modal-header"> <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button> <h4 class="modal-title" id="cssModalLabel">Skin CSS</h4> </div> <div class="modal-body"> <textarea id="getCSSTextarea" class="get-css" readonly></textarea></div> </div> </div> </div> </div>';
        ...
        a.container.find("h4 a").click(function (b) {
            b.preventDefault(), a.container.hasClass("active") ? a.container.animate({bottom: "-" + a.container.height() + "px"}, 300).removeClass("active") : a.container.animate({bottom: "0"}, 300).addClass("active")
        ...
        a || $.cookie("skin") == this.container.find("ul[data-type=colors] li:first a").attr("data-color-hex") ? b.attr("src", "assets/images/logo-default.png") : b.attr("src", "assets/images/logo.png")
    }, reset: function () {
        $.removeCookie("skin"), $.removeCookie("colorGradient"), $.cookie("showSwitcher", !0), window.location.reload()
    }, getCss: function () {
        raw = "", $("#getCSSTextarea").text($('style[id^="less:"]').text()), $("#getCSSModal").modal("show"), options = {
            indent: "	",
            autosemicolon: !0
