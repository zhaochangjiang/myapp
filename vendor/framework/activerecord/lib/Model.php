...
class Model
{
        ...
        if ($value instanceof \DateTime) {
            ...
            if (!($value instanceof $date_class))
                $value = $date_class::createFromFormat(
                    Connection::DATETIME_TRANSLATE_FORMAT, $value->format(Connection::DATETIME_TRANSLATE_FORMAT), $value->getTimezone()
                );
        }
     ...
     */
    private function set_attributes_via_mass_assignment(array &$attributes,
                                                        $guard_attributes)
    {
     ...
     * @return void
     */
    public function set_relationship_from_eager_load(Model $model = null,
                                                     $name)
    {
        $table = static::table();
    ...
    protected static function get_models_from_cache(array $pks)
    {
        ...
        foreach ($pks as $pk) {
            $options = array(
                'conditions' => static::pk_conditions($pk));
    ...
    public static function find_by_pk($values, $options)
    {
        ...
        if ($table->cache_individual_model) {
            $pks = is_array($values) ? $values : array(
                $values);
