...
class pData
{
    ...
    function addPoints($Values,$SerieName = "Serie1")
    {
        ...
        else $this->Data["Series"][$SerieName]["Data"][] = $Values;
    ...
    function getStandardDeviation($Serie)
    {
        if(isset($this->Data["Series"][$Serie]))
        {
            ...
            $DeviationSum = 0;
            foreach($SerieData as $Key=> $Value) $DeviationSum = $DeviationSum+($Value-$Average)*($Value-$Average);
    ...
    function getCoefficientOfVariation($Serie)
    {
        if(isset($this->Data["Series"][$Serie]))
        {
            ...
            if($StandardDeviation!=0) return($StandardDeviation/$Average);
            else return(NULL);
        }
        else return(NULL);
    }
    ...
    function loadPalette($FileName,$Overwrite = FALSE)
    {
        ...
        if(isset($this->Data["Series"]))
        {
            foreach($this->Data["Series"] as $Key=> $Value)
            {
                ...
                else $this->Data["Series"][$Key]["Color"] = $this->Palette[$ID];
                $ID++;
            }
        }
    }
    ...
    function normalize($NormalizationFactor = 100,$UnitChange = NULL,$Round = 1)
    {
        ...
        for($i = 0; $i<=$MaxVal-1; $i++)
        {
            ...
            foreach($SelectedSeries as $Key=> $SerieName)
            {
                ...
                if($Value!=VOID) $Factor = $Factor+abs($Value);
            }
